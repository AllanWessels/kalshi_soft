...
5. **Research & forecast each due market** — **fan out one Sonnet subagent per due market**
   (`model: sonnet`), each forming an INDEPENDENT probability with strict anti-anchoring (do NOT
   look at the Kalshi price until after the estimate). You (Opus) then fetch the price/asks/bids via
   `scripts/refresh_market.py --ticker T`, compare, and record via `scripts/record_forecast.py`
   passing `--yes-ask/--no-ask/--yes-bid/--no-bid` so spot + limit profitability is computed.
