...
from lib import config, store, schemas, atlas
...
def main() -> int:
    ...
    print(f"  {'ticker':<32} {'side':<4} {'yes':<5} {'fair':<5} {'entry':<6} {'EVnet':<7} cell")
    logged = 0
    with (LEDGER_PATH.open("a") if not args.no_log else open("/dev/null", "a")) as lf:
        for r in recs:
            print(f"  {r['ticker']:<32} {r['side']:<4} {r['market_yes']:<5.2f} "
                  f"{r['calibrated_yes']:<5.2f} {r['entry_limit']:<6.2f} {r['ev_net']:+.3f}  {r['cell']}")
            if args.no_log:
                continue
